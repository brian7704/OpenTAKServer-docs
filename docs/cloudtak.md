...
5. Create a symbolic link to enable the new nginx config file.

```bash
sudo ln -s /etc/nginx/sites-available/cloudtak /etc/nginx/sites-enabled/cloudtak
```

6. Install certbot and nginx `sudo apt install nginx certbot python3-certbot-nginx -y`
7. Get a cert from Let's Encrypt `sudo certbot --nginx`
8. Restart nginx `sudo systemctl restart nginx`
9. Open a browser and enter your CloudTAK server's URL, i.e. https://cloudtak.example.com
